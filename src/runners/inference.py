...
def test(model, evaluator, train_loader, val_loader, test_loader, args, device, emb=None, eval_metric='hits'):
    ...
    pos_train_pred, neg_train_pred, train_pred, train_true = test_func(model, train_loader, device, args, emb=emb,split='train')
    print("get val predictions")
    pos_val_pred, neg_val_pred, val_pred, val_true = test_func(model, val_loader, device, args,emb=emb, split='val')
    print("get test predictions")
    pos_test_pred, neg_test_pred, test_pred, test_true = test_func(model, test_loader, device, args,emb=emb, split='test')

    if eval_metric == 'hits':
        results = evaluate_hits(evaluator, pos_train_pred, neg_train_pred, pos_val_pred, neg_val_pred, pos_test_pred,
                                neg_test_pred, Ks=[20, 50, args.K])
...
def get_preds(model, loader, device, args, emb=None, split=None):
    ...
    for batch_count, indices in enumerate(tqdm(loader)):
        ...
        if args.model == 'BUDDY':
            ...
            y_true.append(curr_labels.view(-1).cpu().to(torch.float))
...
@torch.no_grad()
def get_elph_preds(model, loader, device, args, split=None,emb=None):
    n_samples = get_split_samples(split, args, len(loader.dataset))
    t0 = time.time()
    preds = []
    data = loader.dataset
    ...
    for batch_count, indices in enumerate(loader):
        ...
        batch_node_features = None if node_features is None else node_features[curr_links]
        if args.dblp:
            if not args.use_feature and args.train_node_embedding:
                batch_node_features = batch_emb
                node_features = emb
